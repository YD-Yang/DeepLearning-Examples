# -*- coding: utf-8 -*-
"""
Created on Fri Oct 13 15:31:19 2017
...
"""
generate evelnly spaced sequences with noisy measurements
"""
...
n_timesteps    = 20
n_sequences = every_nth = 8
n_features = 1
n_repeats = 100
noise_level = 0.005
use_censored = True

y_train1,x_train1, y_test1,x_test1,events1 = get_data(n_timesteps, every_nth,n_repeats,noise_level,n_features,use_censored)

print (y_test1.shape, x_test1.shape)

plt.imshow(x_test1[:,:,:].sum(axis=2)>0,interpolation="none",cmap='Accent',aspect='auto')
plt.title('x_test (lagged/deterministic event indicator)')
plt.show()
plt.imshow(y_test1[:,:,0],interpolation="none")
plt.title('y_test[:,:,0] tte')
plt.show()

print ('train shape',y_train1.shape,x_train1.shape)
# (might look like the pattern is different but set n_repeats 
#  lower and you'll see that it's the same):
plt.imshow(x_train1[:,:,:].sum(axis=2)>0,interpolation="none",cmap='Accent',aspect='auto')
plt.title('x_train (lagged/noisy event indicator)')
plt.show()
plt.imshow(y_train1[:,:,0],interpolation="none",aspect='auto')
plt.title('y_train[:,:,0] censored tte')
plt.show()
plt.imshow(y_train1[:,:,1],interpolation="none",cmap='Accent',aspect='auto')
plt.title('y_train[:,:,1] u')
plt.show()

"""
to test the implementation, feed raw weibull data
"""
...
tte_mean_train = np.nanmean(y_train1[:,:,0])
init_alpha = -1.0/np.log(1.0-1.0/(tte_mean_train+1.0) )
init_alpha = init_alpha/np.nanmean(y_train1[:,:,1]) # use if lots of censoring
...
model1.add(GRU(1, input_shape=(n_timesteps, n_features),activation='tanh',return_sequences=True))

model1.add(Dense(2))
model1.add(Lambda(output_lambda, arguments={"init_alpha":init_alpha, 
                                               "max_beta_value":4.0}))

model1.compile(loss=weibull_loss_discrete, optimizer=adam(lr=.01))
...
model1.fit(x_train1, y_train1,
          epochs=80, 
          batch_size=x_train1.shape[0]//10, 
          verbose=2, 
...
plt.imshow(x_test1[:every_nth,:,:].sum(axis=2)>0,interpolation="none",cmap='Accent')
...
predicted = model1.predict(x_train1[:every_nth,:,:])
print(predicted.shape)
plt.imshow(x_train1[:every_nth,:,:].sum(axis=2)>0,interpolation="none",cmap='Accent')
plt.title('x features')
plt.show()
plt.imshow(predicted[:,:,0],interpolation="none")
plt.title('alpha')
...
batch_indx =int(every_nth/2)
a = predicted[batch_indx,:,0]
b = predicted[batch_indx,:,1]
this_x_train = x_train1[batch_indx,:,:].mean(axis=1)
this_x_test =  x_test1[batch_indx,:,:].mean(axis=1)

this_tte_train = y_train1[batch_indx,:,0]
this_tte_test =  y_test1[batch_indx,:,0]
